...
async def openai_plan(req: PlanRequest) -> PlanResult:
    ...
    prompt = (
        ...
        "`notes` muss ausführliches Markdown enthalten und diese Überschriften verwenden (genau so):\n"
        "- ## Kurzfassung (2–5 Sätze, konkret)\n"
        "- ## Schritte (mind. 6 nummerierte Schritte; nutze 1., 2., 3. …)\n"
        "- ## Definition of Done (mind. 7 Punkte als Checkliste; nutze '- [ ]')\n"
        "- ## Risiken (mind. 3 Bulletpoints)\n"
        "- ## Quellen (mind. 3, max. 8 URLs; echte Links)\n"
        "- ## Bilder (mind. 1, max. 3 Bild-URLs; echte Bildlinks, z. B. PNG/JPG/SVG)\n\n"
        "Zusatzregeln für `notes`:\n"
        "- Verwende Aufzählungen (Bullets) und nummerierte Listen.\n"
        "- Keine generischen Floskeln – gib direkt Vorschläge, Beispiele und konkrete nächste Aktionen.\n"
        "- URLs müssen als reine URL im Text stehen (damit die UI sie erkennt).\n\n"
        "Schema (JSON):\n"
        "{\n"
